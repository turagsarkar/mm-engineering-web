...
import { Users, Tag, Truck, Bot, AlertTriangle, BarChart3, Activity } from 'lucide-react'
...
export default async function AdminDashboardPage() {
  ...
  const [
    ...
    supabase.from('suppliers').select('*, brands!inner(ai_do_not_quote)', { count: 'exact', head: true }).eq('ai_approved', true).eq('supplier_status', 'active').eq('brands.ai_do_not_quote', false),
    supabase.from('brands').select('*', { count: 'exact', head: true }).eq('ai_do_not_quote', true),
    supabase.from('brands').select('*', { count: 'exact', head: true }).or(`last_reviewed_at.is.null,last_reviewed_at.lt.${new Date(Date.now() - 180 * 24 * 60 * 60 * 1000).toISOString()}`).eq('review_disabled', false),
  ...
  const stats = [
    ...
    { label: 'Active Suppliers', value: supplierCount ?? 0, icon: Truck, color: 'green', href: '/admin/suppliers' },
    { label: 'AI Eligible', value: aiEligibleCount ?? 0, icon: Bot, color: 'purple', href: '/admin/ai-eligible' },
    { label: 'AI Do Not Quote', value: doNotQuoteCount ?? 0, icon: AlertTriangle, color: 'red', href: '/brands?filter=ai_do_not_quote' },
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-6">
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-6">
          ...
          <div className="flex flex-wrap gap-3">
            ...
            <Link href="/admin/reports" className="inline-flex items-center gap-2 px-4 py-2 border border-gray-300 text-gray-700 text-sm font-medium rounded-lg hover:bg-gray-50 transition-colors">
              <BarChart3 className="h-4 w-4" />
              Activity Reports
            </Link>
            <Link href="/admin/ai-dashboard" className="inline-flex items-center gap-2 px-4 py-2 border border-gray-300 text-gray-700 text-sm font-medium rounded-lg hover:bg-gray-50 transition-colors">
        ...
        {/* 2.12 Admin Filters Panel */}
        <AdminFiltersPanel profiles={profiles ?? []} />
